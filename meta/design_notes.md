...
Now for the actual thing. When `stratagem` is invoked, it'll read the stgmrc and start a repl (unless it's been passed the --oneshot flag, in which case it _won't_), then it will execute the source iff source was provided. The stgmrc is literally just a line-by-line list of options that are passed to `stratagem` exactly as though they were given on the command line in the same order. For example, `stratagem --oneshot --reduction lazy EpicSourceFile` with no stgmrc is precisely the same thing as `stratagem` with the following stgmrc.
